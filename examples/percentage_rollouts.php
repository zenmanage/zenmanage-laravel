...
/*
 * Percentage Rollouts (Laravel Facade)
 *
 * Demonstrates SDK-side percentage rollouts using the Zenmanage facade.
 * When a flag includes a `rollout` configuration, the SDK automatically
 * buckets the context identifier via CRC32B hashing and selects either
 * the rollout target/rules or the fallback target/rules.
 *
 * No manual bucketing is needed — the SDK handles it internally.
 */

echo "=== Percentage Rollouts Example (Laravel Facade) ===\n\n";
...
foreach ($users as $user) {
    ...
    $flag = Zenmanage::withContext($context)->single('premium-feature', false);
...
foreach (['user-alpha', 'user-beta', 'user-gamma'] as $userId) {
    ...
    $flag = Zenmanage::withContext($context)->single('landing-page-variant', 'control');
...
$result1 = Zenmanage::withContext($context)->single('rollout-flag')->isEnabled();
$result2 = Zenmanage::withContext($context)->single('rollout-flag')->isEnabled();
$result3 = Zenmanage::withContext($context)->single('rollout-flag')->isEnabled();
